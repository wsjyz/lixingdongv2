...
OrgModel.prototype.update = function(fn){
    ...
    db.zadd(zaddkey+"-"+org.type,new Date().getTime(),hkeyPrefix+org.id,function(err){
...
OrgModel.listId = function(type,fn){
    db.zrevrange(zaddkey+"-"+type,0,-1,function(err,items){
        if (err) return fn(err);
...
OrgModel.list = function(type,fn){

    OrgModel.listId(type,function(err,orgIds){
        //console.log(orgIds);
...
OrgModel.totalCount = function(type,fn){
    db.zcard(zaddkey+"-"+type,function(err,count){
        if(err) return fn(err);
