...
DataModel.removeFromSSet = function(id,mobile,type,fn){
    db.zrem('lxd-goods-price-'+id,mobile,function(err){
        if(err) return fn(err);
    });
}
...
DataModel.removeFromSSetAndStr = function(id,type,fn){
    db.zrem(ssetkeyPrefix+type,stringkeyPrefix+id,function(err){
        if(err) return fn(err);
        db.del(stringkeyPrefix+id,function(err){
            if(err) return fn(err);
        });
    });
}
DataModel.findAllZSetData = function(id,fn){
    db.zrevrange('lxd-goods-price-'+id,0,-1,'withscores',function(err,items){
        if(err) return fn(err);
        var lists = _.groupBy(items, function(a,b) {
            return Math.floor(b/2);
        });
        fn(null,_.toArray(lists));
    });
}
